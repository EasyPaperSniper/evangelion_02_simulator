...
# class Dog:
p.connect(p.GUI)
plane = p.loadURDF("./unitree_env/unitree_data/plane.urdf")
p.setGravity(0,0,-9.8)
p.setTimeStep(1./500)
#p.setDefaultContactERP(0)
#urdfFlags = p.URDF_USE_SELF_COLLISION+p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
urdfFlags = p.URDF_USE_SELF_COLLISION
quadruped = p.loadURDF("./unitree_env/unitree_data/aliengo/urdf/aliengo.urdf",[0,0,0.48],[0,0,0,1], flags = urdfFlags,useFixedBase=False)


# # enable collision between lower legs
for j in range (p.getNumJoints(quadruped)):
        jointInfo = p.getJointInfo(quadruped,j)
        if jointInfo[1].decode("utf8")[-5:]!='fixed':
            print(jointInfo[1])
        # print(jointInfo[12])
