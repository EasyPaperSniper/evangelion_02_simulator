"""
This is an adapted version of the file pybullet_envs.robot_bases in the
original pybullet framework.
We had to adapt the reset function for our purposes to avoid loading the
robot two times into the simulator.
"""
import pybullet
...
class XmlBasedRobot:
	...
	def addToScene(self, bullet_client, bodies):
		...
		if self.ordered_foot is not None:
			ordered_foot = self.ordered_foot
		else:
			ordered_foot = []
		...
		for i in range(len(bodies)):
			...
			for j in range(self._p.getNumJoints(bodies[i])):
				...
				if part_name[-4:] == 'foot':
					ordered_foot.append(parts[part_name])

				if joint_name== "floating_base"  or joint_name== "imu_joint":
		...
		return parts, joints, ordered_joints, self.robot_body, ordered_foot
...
class BodyPart:
	...
	def angular_velocity(self):
		if self.bodyPartIndex == -1:
			_, (wx, wy, wz) = self._p.getBaseVelocity(self.bodies[self.bodyIndex])
		else:
			(x,y,z), (a,b,c,d), _,_,_,_, (vx, vy, vz), (vr,vp,vy) = self._p.getLinkState(self.bodies[self.bodyIndex], self.bodyPartIndex, computeLinkVelocity=1)
...
class Joint:
	def __init__(self, bullet_client, joint_name, bodies, bodyIndex, jointIndex):
		...
		if self.upperLimit < self.lowerLimit:
			self.lowerLimit = 0.0
			self.upperLimit = 0.0
		self.torqueLimit = jointInfo[10]
		if self.torqueLimit == 0:
			self.torqueLimit = 100000.0
		self.velocityLimit =  jointInfo[11]
		if self.velocityLimit == 0:
			self.velocityLimit = 100000.0
	...
	def get_torque(self):
		_, _ , _, tx = self._p.getJointState(self.bodies[self.bodyIndex],self.jointIndex)
		return tx
	...
	def set_velocity(self, velocity):
		velocity = min(velocity, self.velocityLimit)
		# print(velocity)
		self._p.setJointMotorControl2(self.bodies[self.bodyIndex],self.jointIndex,pybullet.VELOCITY_CONTROL, targetVelocity=velocity, force=self.torqueLimit)

	def set_motor_torque(self, torque): # just some synonyme method
		self.set_torque(torque)

	def set_torque(self, torque):
		torque = min(self.torqueLimit, max(-self.torqueLimit, torque))
